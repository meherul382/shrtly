export default async function handler(req,res){
  ...
  try{
    ...
    const check=await fetch(base+'/rest/v1/links?select=code,url&code=eq.'+encodeURIComponent(code)+'&user_id=eq.'+encodeURIComponent(user.id)+'&deleted_at=is.null&limit=1',{headers:{Authorization:'Bearer '+key,apikey:key}});
    if(!check.ok)return res.status(500).json({error:'Could not verify this link.'});
    if(!(await check.json()).length)return res.status(404).json({error:'Link not found in your dashboard.'});
    const update=await fetch(base+'/rest/v1/links?code=eq.'+encodeURIComponent(code)+'&user_id=eq.'+encodeURIComponent(user.id)+'&deleted_at=is.null',{method:'PATCH',headers:{Authorization:'Bearer '+key,apikey:key,'Content-Type':'application/json'},body:JSON.stringify({url})});
    if(!update.ok)return res.status(500).json({error:'Could not update the destination website.'});
    return res.status(200).json({ok:true,code,url});
  }catch(e){console.error(e);return res.status(500).json({error:'Could not update the destination website.'})}
}
