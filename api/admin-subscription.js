const PLANS = { three_day: 3, weekly: 7, monthly: 30 };
export default async function handler(req, res) {
  ...
  try {
    ...
    let patch = { status: 'rejected', updated_at: new Date().toISOString() };
    if (action === 'approve') {
      const lookup = await fetch(`${base}/rest/v1/subscriptions?id=eq.${encodeURIComponent(id)}&select=plan`, { headers: { Authorization: `Bearer ${key}`, apikey: key } });
      if (!lookup.ok) return res.status(500).json({ error: 'Could not read subscription.' });
      const rows = await lookup.json(); const days = PLANS[rows[0]?.plan];
      if (!days) return res.status(400).json({ error: 'Invalid subscription plan.' });
      const started = new Date(); const ends = new Date(started.getTime() + days * 86400000);
      patch = { status: 'active', started_at: started.toISOString(), ends_at: ends.toISOString(), updated_at: started.toISOString() };
    }
